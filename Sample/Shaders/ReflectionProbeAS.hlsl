...
// TODO:
// Copy from OmniLightAS for the culling etc.
...
bool IsCulled( float3 ls_center, float radius )
{
  ...
  if ( ls_center.z > g_ProbeInfo.w + radius ) return true;
...
NUM_THREADS( 32, 1, 1 )
void ReflectionProbeAS( uint3 group_id : SV_GroupID, uint3 local_id : SV_GroupThreadID )
{
  ...
  if ( meshlet_idx < cmd.MeshletCount )
  {
    ...
    for ( int i = 0; i < 6; i++ )
    {
      ...
      float3 ls_position  = MulQuatVec( kViewOrientations[i], bounds.xyz - g_ProbeInfo.xyz );
