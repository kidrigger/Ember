...
{
  float3                   local_dir   = GetCubeDir( global_invocation_id, g_CubeSide );

  Texture2D                eqrect_tex  = ResourceDescriptorHeap[g_EqrectHandle];
  RWTexture2DArray<float4> output_tex  = ResourceDescriptorHeap[g_OutputCubemapHandle];

  float2                   uv          = SampleSphericalMap( local_dir );
  output_tex[global_invocation_id.xyz] = eqrect_tex.SampleLevel( g_EqrectSampler, uv, 0 );
}
